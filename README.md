<p align="center">
  <img src="assets/system-map.svg" alt="Detailed system map for Understand Knowledge" width="100%">
</p>
...
Research is hard to trust when source collection, interpretation, and conclusions are not visibly connected. **Understand Knowledge** turns that work into an explicit sequence with visible inputs, outputs, review points, and completion evidence.

## Why the repository has this name

The shared `hermes-skill-` prefix identifies this as a portable Hermes workflow package. `understand-knowledge` names the capability directly—understand knowledge—so the repository remains searchable and understandable outside the original AI-OS workspace. The public title is **Understand Knowledge**.
...
| What does it do? | Analyze a Karpathy-pattern LLM wiki and generate a knowledge graph with entities and topic clusters. |
...
![Understand Knowledge system map](assets/system-map.svg)

## Operation sequence

![Understand Knowledge actor and data sequence](assets/operation-sequence.svg)
...
![Illustrative output produced by Understand Knowledge](assets/example-output.svg)
...
> For a narrow, non-sensitive research question, analyze a Karpathy-pattern LLM wiki and generate a knowledge graph with entities and topic clusters. Return the result, the evidence used to verify it, and any limitations or actions that still require approval.
