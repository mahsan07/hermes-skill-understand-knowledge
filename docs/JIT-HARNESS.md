...
This package expresses **Understand Knowledge** as a typed, task-adaptive agent harness. Its concrete capability remains: Analyze a Karpathy-pattern LLM wiki and generate a knowledge graph with entities and topic clusters. The harness contract makes memory, planning, action, capability selection, failure handling, and verification explicit.
...
| Module | Strategy | Responsibility in Understand Knowledge |
| --- | --- | --- |
| Memory | `entity_relationship_memory` | Retain markdown knowledge base plus decisions, observations, and verification evidence needed to resume safely. |
| Planning | `dependency_and_path_traversal` | Transform entity extractor into the ordered, bounded stages defined by this skill. |
...
| Action | `retrieve_normalize_synthesize_cite` | Advance the workflow through explicit and implicit links while preserving stop conditions and user authority. |
